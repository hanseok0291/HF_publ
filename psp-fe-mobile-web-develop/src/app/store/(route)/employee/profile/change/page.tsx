...
export default function Page() {
  ...
  const RenderCheckError = () => {
    const renderErrorMessage = (errorField: keyof ChangePsswordFormValues) => {
      return (
    ...
    if (errors.password) {
      return renderErrorMessage("password");
    }
    if (errors.newPassword) {
      return renderErrorMessage("newPassword");
    }
    return renderErrorMessage("currentPassword");
  };
  ...
  const onSubmit = async (data: ChangePsswordFormValues) => {
    try {
      ...
      if (response.content === true) {
        toast({ description: "비밀번호가 정상적으로 변경 되었습니다." });
      }
      router.back();
    } catch (errors: any) {
      toast({
  ...
  return (
    <MainContainer>
      <CloseHeader onClose={() => handleBack()} title="비밀번호 변경" />
      <form
      ...
      >
        <div className="flex flex-col gap-[24px] justify-between mb-[32px]">
          <div className="flex flex-col gap-[8px]">
            <Label className="text-[16px] font-bold">기존 비밀번호</Label>
