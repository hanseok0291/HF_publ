...
// 당첨 바텀 시트
const BottomSheetMissionFail = ({ open, close }) => {
  ...
  return (
    ...
    >
      <div className={`${styleModal.modalBody} ${styleRettoModal.bottomSheetBody} ${styleRettoModal.rettoMission}`}>
        <h2>미션 대상이 아니었어요!</h2>
        <div>
          <p className={`${styleRettoModal.bodyText} ${styleRettoModal.fail}`}>
            기프티몰에서 5천원 이상 <br />
            구매해야 참여 가능!
          </p>
          <span className={`${styleRettoModal.successSubtext}`}>지난 1주 동안 <b>19,500원 구매</b>했네요</span>
        </div>
        <button type="butotn" className={styleRettoModal.detailView} onClick={handleDropdown}>자세한 내용 보기</button>
        {dropdown &&
...
export default BottomSheetMissionFail;
