...
import { getCollectorManualList } from "@/apis/collector/etc/manualApis";
import { getStoreManualFile } from "@/apis/etc/manualApi";
...
export type ManualCollectorParam = ExtractParam<typeof getCollectorManualList>;

const manual = () => {
  const [currentPage, setCurrentPage] = useState<number>(0); // 현재 페이지
  const form = useForm<ManualCollectorParam>({
    mode: "onChange",
    defaultValues: {
      filterType: "ALL",
      page: currentPage,
  ...
  const { swrResponse } = getCollectorManualList(watch());
  const { totalPages = 0 } =
    getCollectorManualList(watch())?.swrResponse?.data?.content ?? {};

  return (
    <FormProvider {...form}>
      <MainContainer>
        <ArrowHeader headerTitle="매뉴얼" />
