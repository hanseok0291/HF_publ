...
// 당첨 바텀 시트
const BottomSheetPresent_Pub = ({ closeHundredPopup, jewel }) => {
  ...
  return (
    ...
    >
      {/* 유지 성공 */}
      {<div className={`${styleModal.modalBody} ${styleRettoModal.bottomSheetBody} ${styleRettoModal.rettoPresent} ${styleRettoModal.emerald} ${jewelClass}`}>
        <h2>우와 연속 10번이에요!</h2>
        <p>축하 선물이 선물함에 도착했어요.</p>
        <div className={`${styleRettoModal.conWrap}`}>
          <div className={`${styleRettoModal.imgWrap} ${styleRettoModal.ruby}`}>
          </div>
        </div>
...
export default BottomSheetPresent_Pub;
