...
$.runningTime = function (type) {
    ...
    if (time == 0) {
        if (type == "ars") $.alertMessage(errTitle.ars, errMsg.arsInputTimeout, $("#commonAlert"));
        else $.alertMessage(errTitle.selfAuth, errMsg.smsInputTimeout, $("#commonAlert"));
...
$.isExprDigit = function (value) {
    var regExp = /^[0-9]+$/;
    if (regExp.test(value.val())) return false;
...
$.isExprBirthDay = function (value) {
    ...
    if (regExp.test(value.val()) && regExp2.test(value.val())) return false;
...
$.validPwd = function (value) {
    ...
    if (val.substring(0, 3) == val.substring(3, 6)) {
        return false;
    } else if (val.substring(0, 2) == val.substring(2, 4) || val.substring(0, 2) == val.substring(4, 6) || val.substring(2, 4) == val.substring(4, 6)) {
        return false;
...
$.closeAllPopupWindow = function () {
    ...
    } else parent.closeAllLayerPopup();
};
