...
iconCtl = function () {
  ...
  if (_btnN < 1 && _backN < 1) {
    $(".GNB .container").append(
      '<div class="btn_menu"><i class="one"></i><i class="two"></i><i class="three"></i></div>'
    );
    $(".detail .section:first-child .container").prepend(
      '<a href="javascript:back();" title="이전페이지로 이동" class="go_back"></a>'
    );
  }
  ...
  // 상단으로 이동 버튼
  $(".btn_top").on("click keypress", function () {
    if(!$(this).is(".share, .coupon")){
...
setRatio = function () {
  ...
  $(".intro_top .media iframe").css("height", mediaH * 0.562);
};
...
var positionCtl = {
  ...
  notice_tabP: function (obj) {
    ...
    if (windowW < 751 && $(window).scrollTop() > tabNavPos) {
      // 모바일버전
  ...
  aniActive: function (obj) {
    ...
    $(".giftimall .section").each(function () {
      if (
        !$(this).hasClass("animation") &&
...
popup1 = function (url, name) {
  var options =
    "width= 420, height=610, top=30, left=20, status=no, scrollbars=yes, resizable, toolbar=no";
  window.open(url, name, options);
};
popup2 = function (url, name) {
  var options =
    "width= 800, height=700, top=30, left=20, resizable=no, scrollbars=yes, location=no";
  window.open(url, name, options);
};
