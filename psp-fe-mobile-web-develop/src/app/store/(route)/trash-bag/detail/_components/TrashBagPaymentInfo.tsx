...
import DetailListContainer from "@/components/store/waste-sticker/detail/DetailListContainer";
import { PAY_STATUS } from "@/enums/Common.enum";
import {
  formatDateWithTime,
  formatNumberWithCommas
} from "@/utils/formatUtils";
import { TrashBagPaymentDataType } from "./TrashBagDetailClient";

export default function TrashBagPaymentInfo({
  payment,
  trashBagList
}: TrashBagPaymentDataType) {
  const renderPaymentMethodCodeName = (paymentMethodCodeName: string) => {
    console.log(paymentMethodCodeName);
    if (paymentMethodCodeName === "PMT_MEAN_002" || "PMT_MEAN_003") {
      return "은행";
    } else {
      return "카드사";
    }
      ...
      <Fragment key={item.paymentId}>
        ...
        {item.cancelList && item.cancelList.length !== 0 && (
          <div>
            ...
            <HeadLessTable data={trashBagList} />
          </div>
        )}
      </Fragment>
    </DetailListContainer>
