...
/** 공지사항 리스트 조회 */
export const getStoreNoticeList: PageableSWR<
  {
    keyWord: string;
...
> = (params) => useCustomSWR("/v1/store/app/notice", params);

/** 공지사항 상세 조회 */
export const getStoreNoticeId: ApiFunction<
  { noticeId: string },
  {
    noticeId: string;
    title: string;
...
> = ({ noticeId }) =>
  getRequest({
    url: `/v1/store/app/notice/${noticeId}`
  });
